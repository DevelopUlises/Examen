...
export class AuthComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this._script.load('body', '/static/assets/vendors/base/vendors.bundle.js', '/static/assets/demo/demo3/base/scripts.bundle.js')
            .then(() => {
    ...
    signin() {
        ...
        this._authService.login(this.model.email, this.model.password)
            .subscribe(
            data => {
                this.globaldata["currentUser"] = data.json().user;
                this._router.navigate([this.returnUrl]);
            },
            error => {
                this.showAlert('alertSignin');
                this._alertService.error(error.json().non_field_errors[0]);
                this.loading = false;
            });
    }
    ...
    forgotPass() {
        ...
        this._userService.forgotPassword(this.model.email)
            ...
            error => {
                ...
                this._alertService.error(error.json());
                this.loading = false;
            });
    }
